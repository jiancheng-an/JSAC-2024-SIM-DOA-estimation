...
N_x = 4; % Number of antennas in the x-direction
N_y = 4; % Number of antennas in the y-direction
...
zeta_set = 0.7:0.02:1; % Decay rate
d_x = 4 * lambda / 9; % Meta-atom spacing in the x-direction
d_y = 4 * lambda / 9; % Meta-atom spacing in the y-direction
L = 13; % Number of metasurface layers
M_x = 15;
M_y = 15;
M = M_x * M_y; % Number of meta-atoms per layer
d_layer = 12 * lambda / L; % Spacing between two adjacent layers
...
for mm_zeta = 1:length(zeta_set)
    tic
    rng(5); % Increases from 1 to 5
    %% Initialization
    ...
    for jj = 1:100
        learning_rate = learning_rate * zeta_set(mm_zeta);
        for ll = 1 : L
            for mm1 = 1 : M
                X_left = W_T_0;
                for ll_left = 1 : (ll - 1)
    ...
    mm_zeta
    toc
    NMSE(mm_zeta) = nmse_temp(end);
end
figure;
semilogy(nmse_temp);
figure;
semilogy(zeta_set, NMSE)
NMSE_5 = NMSE;
save NMSE_5 NMSE_5