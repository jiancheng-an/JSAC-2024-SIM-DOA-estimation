...
clearvars;
close all;
M_set = [13 13 13 14 14 14 15 15 15];
Thick_set = [10 11 12 10 11 12 10 11 12];
Spacing_set = [8 8 8 9 9 9 10 10 10];
L_set = [11 12 13 11 12 13 11 12 13];
...
for ii_counter = 1:2
    for jj_counter = 1:2
        tic
        rng(1);
        d_x = 4 * lambda / Spacing_set(jj_counter); % Meta-atom spacing in the x-direction
        d_y = 4 * lambda / Spacing_set(jj_counter); % Meta-atom spacing in the y-direction
        ...
        for jj = 1 : 100
            learning_rate = learning_rate * 0.95; % Decay rate: 0.95
             for ll = 1 : L
                for mm1 = 1 : M
                    X_left = W_T_0;
                    for ll_left = 1 : (ll - 1)
